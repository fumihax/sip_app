...
param の変数に従って，mesg へ SDPボディを格納する．

@param param : SIP用パラメータ
@param mesg  : 転送用の SIPメッセージへのポインタ．
...
param の変数に従って，mesg へ SDPボディを格納する もう一つの方法．分かり易いがスマートではない．
...
SDPボディから media の keytype を取り出す．

@param sdp : SDPボディへのポインタ．
@param pos : 何番目のメディアかを指定する．通常は 0．

@retval keytype を示す文字列．freeする必要あり．
@retval エラーの場合は NULL
*/
char*  sdp_get_media_keytype(sdp_message_t* sdp, int pos) 
{
...
SDPボディから media の keydata を取り出す．

@param sdp : SDPボディへのポインタ．
@param pos : 何番目のメディアかを指定する．通常は 0．

@retval keydata を示す文字列．freeする必要あり．
@retval エラーの場合は NULL
*/
...
SDPボディから media の 属性の値を取り出す．

@param sdp : SDPボディへのポインタ．
@param key : 属性(attribute) を指定する．
@param pos : 何番目のメディアかを指定する．通常は 0．

@retval 属性の値を示す文字列．freeする必要あり．
@retval 属性が無い．またはエラーの場合は NULL
*/
char*  sdp_get_media_attr(sdp_message_t* sdp, char* key, int pos) 
{
    int   i;
...
SDPボディの内容の一部を取り出してプリントする．デバッグ用． 取り出し方法の確認用．

@param sdp : SDPボディへのポインタ． see /usr/local/include/osipparser2/sdp_message.h 
*/
void  sdp_show_body(sdp_message_t* sdp)
{
    int i, j;
...
SDPボディの内容をプリントする．デバッグ用．sdp_show_body() と違い，ただ垂れ流すだけ．
